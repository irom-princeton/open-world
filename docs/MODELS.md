...
| file | views | action input | inference config | aux state head |
|---|---|---|---|---|
| `wm_student_2view.pt` | 2 (side + wrist) | cartesian absolute, 7-d | `configs/inference/ar_wan_student_2view.py` | 8-d |
| `wm_student_3view_bimanual.pt` | 3 (scene + 2 wrists) | cartesian absolute, 20-d | `configs/inference/ar_wan_student_3view_bimanual.py` | 16-d |
...
interchangeable (different view count, action dimensionality, and state-pred head
size). See [configs/inference/README.md](../configs/inference/README.md).
