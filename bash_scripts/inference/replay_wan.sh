...
#SBATCH --output=slurm_outputs/ar_replay_wan/%j.out
#SBATCH --error=slurm_outputs/ar_replay_wan/%j.out
#
# Stage 3 (inference) — open-loop trajectory replay with the Wan student. Primes
# the model with ground-truth history, feeds the recorded action sequence, and
# generates the rest; writes side-by-side GT|PRED videos + an MSE/PSNR summary.
#
...
CONFIG="${CONFIG:-configs/training/ar_wan_droid.py}"
