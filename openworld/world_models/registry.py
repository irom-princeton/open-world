...
Exposes the **SVD bidirectional** model (``vidwm``), the **Ctrl-World** adapter
(``ctrlworld``), a ``dummy`` stub, the **autoregressive Wan student** (``ar_wan``),
and the **WEAVER** model (``weaver``). Every non-dummy entry is registered *lazily*
(see the ``_register_*`` helpers) so that requesting one model never imports another
...
def _register_ctrlworld() -> None:
    """Lazily register the Ctrl-World adapter (SVD dep set, same as vidwm)."""
    if "ctrlworld" in WORLD_MODEL_REGISTRY:
        return
    from openworld.world_models.ctrl_world_world_model import CtrlWorldWorldModel
    WORLD_MODEL_REGISTRY["ctrlworld"] = CtrlWorldWorldModel
...
_LAZY = {"vidwm": _register_vidwm, "ctrlworld": _register_ctrlworld,
         "ar_wan": _register_ar, "weaver": _register_weaver}
